...
class Trainer(object):
    def __init__(self, data_api):
        ...
        self.dlsa_f1_score_trainer = F1ScoreTrainer(data_api)
        self.vision_f1_score_trainer = F1ScoreTrainer(data_api)
    ...
    def train(self,
        ...
        dlsa_train_output = process_dlsa_output(
            dlsa_train_output, dlsa_train_input, pd)
        vision_train_output = process_vision_output(
            vision_train_output, pd)
    ...
    def predict(self,
        ...
        dlsa_processed_output = process_dlsa_output(
            dlsa_predict_output, dlsa_predict_input, pd)
        vision_processed_output = process_vision_output(
            vision_predict_output, pd)
        ...
        if (len(dlsa_scored_pred) != len(vision_scored_pred)) or (
            ...
            dlsa_scored_pred, vision_scored_pred = set_diff_ids(
                dlsa_scored_pred, vision_scored_pred, pd
            )
...
def set_diff_ids(df1, df2, pd):
    ...
    if diff_ids_1:
        df2 = (
            pd.concat([df2, df1[df1.id.isin(diff_ids_1)]])
            .sort_values(by="id")
    ...
    if diff_ids_2:
        df1 = (
            pd.concat([df1, df2[df2.id.isin(diff_ids_2)]])
            .sort_values(by="id")
            .reset_index(drop=True)
        )

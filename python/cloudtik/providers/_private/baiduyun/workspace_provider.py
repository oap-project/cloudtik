...
from cloudtik.providers._private.baiduyun.config import create_baiduyun_workspace, \
    delete_baiduyun_workspace, check_baiduyun_workspace_integrity, update_baiduyun_workspace_firewalls, \
    get_workspace_head_nodes, list_baiduyun_clusters, bootstrap_baiduyun_workspace, check_baiduyun_workspace_existence, \
    get_baiduyun_workspace_info
...
class BaiduyunWorkspaceProvider(WorkspaceProvider):
    ...
    def create_workspace(self, config):
        create_baiduyun_workspace(config)

    def delete_workspace(self, config,
                         delete_managed_storage: bool = False):
        delete_baiduyun_workspace(config, delete_managed_storage)
    
    def update_workspace_firewalls(self, config):
        update_baiduyun_workspace_firewalls(config)

    def check_workspace_existence(self, config: Dict[str, Any]):
        return check_baiduyun_workspace_existence(config)

    def check_workspace_integrity(self, config):
        return check_baiduyun_workspace_integrity(config)

    def list_clusters(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        return list_baiduyun_clusters(config)
    ...
    def get_workspace_info(self, config: Dict[str, Any]):
        return get_baiduyun_workspace_info(config)

    @staticmethod
    def bootstrap_workspace_config(config):
        return bootstrap_baiduyun_workspace(config)

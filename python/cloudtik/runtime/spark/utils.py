...
SPARK_METASTORE_SERVICE_SELECTOR_KEY = "metastore_service_selector"
...
def _config_depended_services(cluster_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if spark_config.get(SPARK_HDFS_NAMENODE_URI_KEY) is None:
        if _is_hdfs_service_discovery(spark_config):
            hdfs_namenode_uri = discover_hdfs(
                spark_config, SPARK_HDFS_SERVICE_SELECTOR_KEY,
...
def _discover_hdfs_on_head(cluster_config: Dict[str, Any]):
    ...
    if not _is_hdfs_service_discovery(spark_config):
        return cluster_config

    hdfs_namenode_uri = spark_config.get(SPARK_HDFS_NAMENODE_URI_KEY)
...
def _is_valid_storage_config(config: Dict[str, Any], final=False):
    ...
    if (not final and _is_hdfs_service_discovery(spark_config) and

...
from test_k8s_operator_basic import get_kubernetes_tool_path
...
def delete_cloudtikclusters(namespace):
    cmd = f"kubectl -n {namespace} delete cloudtikclusters --all"
    try:
...
class HelmTest(unittest.TestCase):
    def test_helm(self):
        charts_dir = get_kubernetes_tool_path("helm/charts")
        os.chdir(charts_dir)
        ...
        print(">>>Uninstalling...")
        delete_cloudtikclusters(namespace=NAMESPACE)
        ...
        print(">>>Testing installation of multiple clusters.")
        print(">>>Installing...")
        helm(NAMESPACE, "install", "cloudtik-operator", operatorOnly="true")
        wait_for_pods(1, namespace=f"{NAMESPACE}2")
        ...
        helm(
            ...
            "install",
            "example-cluster2",
            **{"clusterOnly": "true", "podTypes.workerDefault.minWorkers": "0"},
        )
        ...
        wait_for_pods(0, namespace=NAMESPACE)
        helm(NAMESPACE, "uninstall", "cloudtik-operator")
        wait_for_pods(0, namespace=f"{NAMESPACE}2")

        # namespacedOperator
        print(">>>Testing installation of namespaced operator.")
        print(">>>Installing...")

...
case $key in
    ...
    -h|--help)
    shift 1 # past argument
    echo "Usage: spark-pi.sh --api-server k8s-api-server-endpoint --namespace k8s-namespace --driver-service-account spark-driver-service-account --executor-service-account spark-executor-service-account --image image:tag --spark_conf spark-conf-dir --help other spark configurations"
    exit 1
    ;;

...
function prepare_tpcds_queries(){
    if [ ! -d "/tmp/trino" ]; then
        git clone https://github.com/trinodb/trino.git /tmp/trino
    fi
    rm -rf $TRINO_HOME/tpcds/tpcds-queries && mkdir -p $TRINO_HOME/tpcds
    cp -r /tmp/trino/testing/trino-benchto-benchmarks/src/main/resources/sql/presto/tpcds $TRINO_HOME/tpcds/tpcds-queries
    database=tpcds
    schema=${SCALE}
    prefix=""
    sed -i "s#\${database}#'${database}'#g" `grep '\${database}' -rl $TRINO_HOME/tpcds`
    sed -i "s#\${schema}#'${schema}'#g" `grep '\${schema}' -rl $TRINO_HOME/tpcds`
...
function prepare_tpch_queries(){
    ...
    cp -r /tmp/trino/testing/trino-benchto-benchmarks/src/main/resources/sql/presto/tpch $TRINO_HOME/tpch/tpch-queries
    database=tpch
    schema=${SCALE}
    prefix=""
    sed -i "s#\${database}#'${database}'#g" `grep '\${database}' -rl $TRINO_HOME/tpch`
    sed -i "s#\${schema}#'${schema}'#g" `grep '\${schema}' -rl $TRINO_HOME/tpch`
